...
for i in video_game_sales:
    genre.append(i[4])
    global_sales.append(i[9])
    
    sales_by_genre = {}
    for k, v in zip(genre, global_sales):
        sales_by_genre[k] = sales_by_genre.get(k, 0) + v
print(sales_by_genre)
#---b---
publisher = []
for r in video_game_sales:
    publisher.append(r[5])
games_per_publisher = {item: publisher.count(item) for item in set(publisher)}
print(games_per_publisher)
#---c---
#key = ['name','year','genre','publisher','global_sales']
#game = video_game_sales[0]
#rank_1 = [game[1], game[3], game[5], game[6], game[9]]
#top_game = dict(zip(key,rank_1))
#items = list(top_game.items())
#for r in items: 
#    print(r)
